...
-- can't unload the codemap data while another thread is in an `inCodemap` block.
...
main = do
    ...
    _thread <- forkIO $ do
        takeMVar var1
        -- When doing codemapIsLoaded the check here, we also verify that other
        -- threads can access the codemap info. But I'm not sure how to test that
        -- a different unix thread can access the codemap info as well.
        putStrLn "3"
