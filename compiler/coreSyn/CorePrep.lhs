...
tryEtaReducePrep bndrs (Tick tickish e)
  = fmap (Tick tickish) $ tryEtaReducePrep bndrs e
...
-- into the floating binds (here "a") in cpePair. This is implemented
-- in @wrapTicks@.
...
wrapTicks (Floats flag floats0) expr = (Floats flag floats1, expr')
  where (floats1, expr') = foldrOL go (nilOL, expr) floats0
        ...
        wrapBind t (NonRec binder rhs) = NonRec binder (Tick t rhs)
        wrapBind t (Rec pairs)         = Rec (mapSnd (Tick t) pairs)
